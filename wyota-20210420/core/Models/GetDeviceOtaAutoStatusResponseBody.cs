// This file is auto-generated, don't edit it. Thanks.

using System;
using System.Collections.Generic;
using System.IO;

using Tea;

namespace AlibabaCloud.SDK.Wyota20210420.Models
{
    public class GetDeviceOtaAutoStatusResponseBody : TeaModel {
        [NameInMap("Code")]
        [Validation(Required=false)]
        public string Code { get; set; }

        [NameInMap("Data")]
        [Validation(Required=false)]
        public GetDeviceOtaAutoStatusResponseBodyData Data { get; set; }
        public class GetDeviceOtaAutoStatusResponseBodyData : TeaModel {
            [NameInMap("AutoUpdate")]
            [Validation(Required=false)]
            public int? AutoUpdate { get; set; }

            [NameInMap("AutoUpdateTimeSchedule")]
            [Validation(Required=false)]
            public string AutoUpdateTimeSchedule { get; set; }

            [NameInMap("ForceUpgrade")]
            [Validation(Required=false)]
            public int? ForceUpgrade { get; set; }

            [NameInMap("Status")]
            [Validation(Required=false)]
            public int? Status { get; set; }

        }

        [NameInMap("Message")]
        [Validation(Required=false)]
        public string Message { get; set; }

        [NameInMap("RequestId")]
        [Validation(Required=false)]
        public string RequestId { get; set; }

    }

}
