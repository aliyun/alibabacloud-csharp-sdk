...
namespace AlibabaCloud.SDK.Ims20190815.Models
{
    public class ListVirtualMFADevicesRequest : TeaModel {
        ...
        /// <para>When you call the API for the first time, if the total number of entries exceeds the <c>MaxItems</c> limit, the data is truncated and only <c>MaxItems</c> entries are returned. In this case, the <c>IsTruncated</c> response parameter is <c>true</c> and a <c>Marker</c> is returned. You can use the <c>Marker</c> returned from the previous call to continue calling the API with the same request parameters to query the truncated data. You can repeat this process until <c>IsTruncated</c> is <c>false</c>, which indicates that all data has been retrieved.</para>
        /// 
        /// <b>Example:</b>
        /// <para>EXAMPLE</para>
        /// </summary>
