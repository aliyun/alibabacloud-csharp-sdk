...
namespace AlibabaCloud.SDK.Ecs20140526.Models
{
    public class InvokeCommandRequest : TeaModel {
        ...
        /// </description></item>
        /// <item><description><para>If you specify this parameter, the command can run only on Linux instances with Cloud Assistant Agent version 2.2.3.344 or later.</para>
        /// <list type="bullet">
        ...
        /// </list>
        /// </description></item>
        /// <item><description><para>If you specify this parameter, the <c>Username</c> parameter specified in this operation and the <c>WorkingDir</c> parameter specified in <a href="https://help.aliyun.com/document_detail/64844.html">CreateCommand</a> do not take effect. The command is executed only by the default user in the default working directory of the container. For more information, see <a href="https://help.aliyun.com/document_detail/456641.html">Use Cloud Assistant to run commands in containers</a>.</para>
        /// </description></item>
        ...
        /// <item><description>The execution interval is based on a fixed frequency and is independent of the actual execution time of the task. For example, if the command is set to execute every 5 minutes and the task takes 2 minutes to complete, the next execution starts 3 minutes after the task completes.</description></item>
        /// <item><description>The command is not executed immediately when the task is created. For example, if the command is set to execute every 5 minutes, the first execution starts 5 minutes after the task is created.</description></item>
        /// </list>
        /// </description></item>
        ...
        /// <item><description>Time zone offset from Greenwich Mean Time: For example, <c>GMT+8:00</c> (UTC+8) or <c>GMT-7:00</c> (UTC-7). When using the GMT format, leading zeros are not supported for the hour value.</description></item>
        /// <item><description>Time zone abbreviation: Only UTC (Coordinated Universal Time) is supported.</description></item>
        /// </list>
        /// <para>For example, to execute once at 13:15:30 on June 6, 2022 in China/Shanghai time, use: <c>at(2022-06-06 13:15:30 Asia/Shanghai)</c>. To execute once at 13:15:30 on June 6, 2022 in UTC-7, use: <c>at(2022-06-06 13:15:30 GMT-7:00)</c>.</para>
        /// </description></item>
        /// <item><description><para>Clock-based scheduling (based on a Cron expression): Based on a Cron expression, the command is executed on a set schedule. Format: <c>&lt;seconds&gt; &lt;minutes&gt; &lt;hours&gt; &lt;day of month&gt; &lt;month&gt; &lt;day of week&gt; &lt;year (optional)&gt; &lt;time zone&gt;</c>. The scheduled execution time is calculated based on the Cron expression in the specified time zone. If no time zone is specified, the system time zone of the instance is used by default. For more information about Cron expressions, see <a href="https://help.aliyun.com/document_detail/64769.html">Cron expressions</a>. The time zone supports the following three formats:</para>
        /// <list type="bullet">
        /// <item><description>Full time zone name: For example, <c>Asia/Shanghai</c> (China/Shanghai time) or <c>America/Los_Angeles</c> (US/Los Angeles time).</description></item>
        /// <item><description>Time zone offset from Greenwich Mean Time: For example, <c>GMT+8:00</c> (UTC+8) or <c>GMT-7:00</c> (UTC-7). When using the GMT format, leading zeros are not supported for the hour value.</description></item>
        /// <item><description>Time zone abbreviation: Only UTC (Coordinated Universal Time) is supported.
        /// For example, to execute once at 10:15 every day in 2022 in China/Shanghai time, use <c>0 15 10 ? * * 2022 Asia/Shanghai</c>. To execute every 30 minutes from 10:00 to 11:30 every day in 2022 in UTC+8, use <c>0 0/30 10-11 * * ? 2022 GMT+8:00</c>. To execute every 5 minutes from 14:00 to 14:55 every day in October every two years starting from 2022 in UTC, use <c>0 0/5 14 * 10 ? 2022/2 UTC</c>.</description></item>
        /// </list>
        /// <remarks>
        ...
        /// <summary>
        /// <para>The OSS delivery configuration for the command execution output.</para>
        /// <list type="bullet">
        ...
        /// <item><description>After Base64 encoding, the total length of the custom parameters and the original command content cannot exceed 24 KB.</description></item>
        ...
        /// <summary>
        /// <para>Specifies how the command is executed. Valid values:</para>
        /// <list type="bullet">
        ...
        /// <item><description>DryRun: performs a dry run of the request without actually executing the command. The dry run checks request parameters, the instance execution environment, and the Cloud Assistant Agent status.</description></item>
        /// </list>
        /// <para>Default value:</para>
        /// <list type="bullet">
        /// <item><description>If you do not specify the <c>Frequency</c> parameter, the default value is <c>Once</c>.</description></item>
        /// <item><description>If you specify the <c>Frequency</c> parameter, the command is executed as <c>Period</c> regardless of whether you set this parameter.</description></item>
        /// </list>
        /// <para>Precautions:</para>
        /// <list type="bullet">
        /// <item><description>You can call <a href="https://help.aliyun.com/document_detail/64838.html">StopInvocation</a> to stop a pending or scheduled command execution.</description></item>
        ...
        /// <summary>
        /// <para>The tags used to filter instances. You can use tags to run commands in batches on instances with the same tag without specifying InstanceId.</para>
        /// </summary>
        ...
        public class InvokeCommandRequestTag : TeaModel {
            /// <summary>
            /// <para>The tag key for the command execution. Valid values of N: 1 to 20. The tag key cannot be an empty string.</para>
            /// <para>If you use a single tag to filter resources, the number of resources with this tag cannot exceed 1,000. If you use multiple tags to filter resources, the number of resources with all specified tags attached cannot exceed 1,000. If the number of resources exceeds 1,000, execute the <a href="https://help.aliyun.com/document_detail/110425.html">ListTagResources</a> operation to query resources.</para>
            ...
            /// <summary>
            /// <para>The tag value for the command execution. Valid values of N: 1 to 20. The tag value can be an empty string.</para>
        ...
        /// </description></item>
        /// <item><description><para>If the command cannot run because of process issues, missing modules, or missing Cloud Assistant Agent, a timeout occurs. When a timeout occurs, the command process is forcefully terminated.</para>
        /// </description></item>
        /// <item><description><para>If you do not set this value, the timeout period specified when the command was created is used.</para>
        /// </description></item>
        ...
        /// <summary>
        /// <para>The username used to execute the command on the ECS instance. The value can be up to 255 characters in length.</para>
        /// <list type="bullet">
        ...
        /// <summary>
        /// <para>The name of the password used by the user to execute the command on a Windows instance. The value can be up to 255 characters in length.</para>
        /// <para>If you want to execute a command as a non-default user (System) on a Windows instance, you must specify both <c>Username</c> and this parameter. To reduce the risk of password leaks, store the plaintext password in the parameter repository of operations management, and pass in only the password name here. For more information, see <a href="https://help.aliyun.com/document_detail/186828.html">Encryption parameters</a> and <a href="https://help.aliyun.com/document_detail/203771.html">Configure a regular user to run Cloud Assistant commands</a>.</para>
        /// <remarks>
        ...
        /// <summary>
        /// <para>The directory in which the command is executed on the ECS instance. The value can be up to 200 characters in length.</para>
        /// <list type="bullet">
        /// <item><description>If you do not set this value, the working directory specified when the command was created is used.</description></item>
